...
for (path, loc, cavg, cmax, churn, debt, grade, crit, maj, mi) in modules_data:
    db.add(Module(
        id=str(uuid.uuid4()), job_id=JOB_ID, path=path, loc=loc,
        complexity_avg=cavg, complexity_max=cmax, churn_score=churn,
        debt_score=debt, grade=grade,
        issue_count_critical=crit, issue_count_major=maj, issue_count_minor=mi,
    ))

# Misconfigs
misconfigs_data = [
    ("checkov", "Dockerfile", "Dockerfile", 8, "CKV_DOCKER_2", "Ensure that HEALTHCHECK instructions have been added", "major", "Add: HEALTHCHECK CMD curl -f http://localhost:8000/health || exit 1"),
    ("hadolint", "Dockerfile", "Dockerfile", 3, "DL3008", "Pin versions in apt-get install to avoid breaking changes", "minor", "Replace: apt-get install -y python3 → apt-get install -y python3=3.11*"),
    ("tfsec", "Terraform", "infra/main.tf", 45, "aws-s3-no-public-access", "S3 bucket allows public read access", "critical", "Add: acl = \"private\" and block_public_acls = true"),
    ("env-checker", "EnvFile", ".env", 3, "WEAK_CONFIG", "DEBUG mode enabled — disable in production", "major", "Set: DEBUG=False"),
    ("env-checker", "EnvFile", ".env", 7, "ENV_NOT_GITIGNORED", ".env is not in .gitignore — secrets may be committed", "critical", "Add .env to .gitignore immediately"),
    ("checkov", "K8s", "k8s/deployment.yaml", 22, "CKV_K8S_28", "Do not admit containers with the NET_RAW capability", "major", "Add securityContext.capabilities.drop: [NET_RAW]"),
]
for (tool, rt, fp, ls, cid, title, sev, rem) in misconfigs_data:
    db.add(Misconfig(
        id=str(uuid.uuid4()), job_id=JOB_ID, tool=tool, resource_type=rt,
        file_path=fp, line_start=ls, check_id=cid, title=title,
        severity=sev, remediation=rem,
    ))

# Dep updates
dep_updates_data = [
    ("pip", "requests", "2.25.1", "2.32.3", "MANDATORY", ["CVE-2023-32681"], "Fixes SSRF via Proxy-Authorization header leak. Safe to apply — no breaking changes.", "pip install requests==2.32.3"),
    ("pip", "setuptools", "57.0.0", "70.3.0", "MANDATORY", ["CVE-2022-40897"], "Fixes ReDoS in package_index.py. Straightforward update.", "pip install setuptools==70.3.0"),
    ("pip", "cryptography", "3.4.8", "42.0.8", "SUGGESTED", [], "Multiple security improvements. API compatible for our usage.", "pip install cryptography==42.0.8"),
    ("pip", "pillow", "8.3.2", "10.4.0", "SUGGESTED", ["CVE-2023-44271"], "Fixes uncontrolled resource consumption in ImageFont. Update recommended.", "pip install pillow==10.4.0"),
    ("pip", "flask", "2.0.3", "3.0.3", "INFORMATIONAL", [], "Major version bump — review breaking changes in Flask 3.x migration guide before updating.", "pip install flask==3.0.3"),
    ("pip", "black", "21.12b0", "24.4.2", "OPTIONAL", [], "Formatter update only — no security implications.", "pip install black==24.4.2"),
]
for (eco, pkg, cur, lat, cls, cves, summary, cmd) in dep_updates_data:
    db.add(DepUpdate(
        id=str(uuid.uuid4()), job_id=JOB_ID, ecosystem=eco,
        package_name=pkg, current_version=cur, latest_version=lat,
        classification=cls, cve_ids=cves, changelog_summary=summary, upgrade_command=cmd,
    ))

# Advisories
db.add(Advisory(
    id=str(uuid.uuid4()), source="nvd", advisory_id="CVE-2023-32681",
    package_name="requests", ecosystem="PyPI", severity="major", cvss_score=6.1,
    title="CVE-2023-32681: requests — Proxy-Authorization header leak via redirect",
    description="Requests forwarded Proxy-Authorization headers to destination servers on redirect.",
    fixed_version="2.31.0", advisory_url="https://nvd.nist.gov/vuln/detail/CVE-2023-32681",
    affects_project=True, is_read=False,
))
db.add(Advisory(
    id=str(uuid.uuid4()), source="github", advisory_id="GHSA-j8r2-6x86-q33q",
    package_name="cryptography", ecosystem="PyPI", severity="major", cvss_score=7.5,
    title="GHSA-j8r2-6x86-q33q: cryptography — NULL pointer dereference in PKCS12",
    description="The cryptography package before 41.0.0 is vulnerable to NULL pointer dereference via PKCS12.",
    fixed_version="41.0.0", advisory_url="https://github.com/advisories/GHSA-j8r2-6x86-q33q",
    affects_project=True, is_read=False,
))

# Compliance
owasp_results = [
    ("owasp", "A01:2021-Broken Access Control", "A01:2021-Broken Access Control", "fail", 2),
    ("owasp", "A02:2021-Cryptographic Failures", "A02:2021-Cryptographic Failures", "fail", 4),
    ("owasp", "A03:2021-Injection", "A03:2021-Injection", "fail", 3),
    ("owasp", "A04:2021-Insecure Design", "A04:2021-Insecure Design", "partial", 1),
    ("owasp", "A05:2021-Security Misconfiguration", "A05:2021-Security Misconfiguration", "fail", 3),
    ("owasp", "A06:2021-Vulnerable Components", "A06:2021-Vulnerable Components", "fail", 4),
    ("owasp", "A07:2021-Identification Failures", "A07:2021-Identification Failures", "fail", 1),
    ("owasp", "A08:2021-Software Integrity", "A08:2021-Software Integrity", "pass", 0),
    ("owasp", "A09:2021-Logging Failures", "A09:2021-Logging Failures", "partial", 1),
    ("owasp", "A10:2021-SSRF", "A10:2021-SSRF", "fail", 1),
    ("nist", "PR.DS-1", "Data-at-rest protection", "fail", 3),
    ("nist", "DE.CM-8", "Vulnerability scans", "pass", 0),
]
for (fw, cid, cname, status, count) in owasp_results:
    db.add(ComplianceResult(
        id=str(uuid.uuid4()), job_id=JOB_ID, framework=fw,
        control_id=cid, control_name=cname, status=status, issue_count=count,
    ))
...
print(f"   Open: http://localhost:7860?demo={JOB_ID}")
print(f"   API:  http://localhost:7860/report/{JOB_ID}\n")
